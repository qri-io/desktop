...
describe('Qri End to End tests', function spec () {
  ...
  const metaCommitTitle = 'edited meta'
  const metaCommitMessage = 'added all standard metadata'
  const bodyCommitTitle = 'remove rows'
  const bodyCommitMessage = 'changed body using fsi'

  const metaTitle = 'Earthquakes'
  ...
  const metaReadmeURL = 'https://earthquake.usgs.gov/earthquakes/feed/v1.0/csv.php'
  const metaAccrualPeriodicity = 'Monthly'
  const metaVersion = '1.0.0'
  const metaLanguage = 'english'
  const metaIdentifier = '10000000'
  ...
  it('create a meta & commit', async () => {
    ...
    // on dataset page
    await click('#workbench', artifactPath('create-a-meta-and-commit-click-workbench.png'))
    // on status tab
    await click('#status-tab', artifactPath('create-a-meta-and-commit-click-status-tab.png'))
    // commit should be disabled
    await doesNotExist('.clear-to-commit #commit-status', artifactPath('create-a-meta-and-commit-does-not-exist-clear-to-commit.png'))
    // click #meta-status
    await click('#meta-status', artifactPath('create-a-meta-and-commit-click-meta-status.png'))
    ...
    await expectTextToBe('#theme-tag-0', metaTheme)

    // set 2 keywords
    await setValue('#keywords-tag-input', metaKeywords[0])
    await sendKeys('#keywords-tag-input', "Tab")
    await delay(500)

    await waitForExist('#keywords-tag-0')
    await expectTextToBe('#keywords-tag-0', metaKeywords[0])
    await setValue('#keywords-tag-input', metaKeywords[1])
    await sendKeys('#keywords-tag-input', "Tab")
    await delay(500)
    ...
    await setValue('#downloadURL', metaDownloadURL)
    await delay(500)
    // set value for homeURL
    await setValue('#homeURL', metaHomeURL)
    await delay(500)
    ...
    await setValue('#version', metaVersion)
    await delay(500)
    // set value for identifier
    await setValue('#identifier', metaIdentifier)
    await delay(500)

    await checkStatus('meta', 'added', artifactPath('create-a-meta-and-commit-check-status-added.png'))

    if (takeScreenshots) {
      await takeScreenshot(artifactPath('csv-workbench-meta-edit.png'))
    }

    // commit should be enabled
    await waitForExist('.clear-to-commit #commit-status', artifactPath('create-a-meta-and-commit-exists-clear-to-commit.png'))
    // click #commit-status
    await click('#commit-status', artifactPath('create-a-meta-and-commit-click-commit-status.png'))
    // set title and message

    await setValue('#title', metaCommitTitle, artifactPath('create-a-meta-and-commit-set-value-title2.png'))
    await setValue('#message', metaCommitMessage, artifactPath('create-a-meta-and-commit-set-value-description2.png'))

    if (takeScreenshots) {
      await takeScreenshot(artifactPath('csv-workbench-meta-edit-commit.png'))
    }
    // submit
    await click('#submit', artifactPath('create-a-meta-and-commit-click-submit.png'))
    // on history tab
    await onHistoryTab(artifactPath('create-a-meta-and-commit-on-history-tab.png'))
    // meta status should be 'added'
    await checkStatus('meta', 'added', artifactPath('create-a-meta-and-commit-check-status-meta-added2.png'))
    // commit title should be the same
    await expectTextToBe('#commit-title', metaCommitTitle, artifactPath('create-a-meta-and-commit-expect-text-to-be-commit-title.png'))
    // verify meta title and description should be correct
    await click('#meta-status', artifactPath('create-a-meta-and-commit-click-meta-status2.png'))
    ...
    await expectTextToBe('#meta-accrualPeriodicity', metaAccrualPeriodicity)
    await expectTextToBe('#meta-version', metaVersion)
    await expectTextToBe('#meta-identifier', metaIdentifier)
  })
