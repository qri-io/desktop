...
import Dataset, { Commit, Meta } from '../../app/models/dataset'
...
async function commitEdits (uniqueName: string, commitTitle: string, commitMessage: string | undefined, utils: E2ETestUtils, imagesDir: string) {
  ...
  await checkStatus('meta', 'added', artifactPathFromDir(imagesDir, `${name}-commit-check-status-meta-added.png`))
...
async function editMeta (uniqueName:string, dataset: Dataset, status: 'added' | 'modified' | 'removed', utils: E2ETestUtils, imagesDir: string) {
  ...
  // commit!
  await commitEdits('fsi-meta-edit', commit.title, commit.message, utils, imagesDir)
